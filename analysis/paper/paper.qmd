...
<!--  introduce some key concepts that we use later such as SSDA -->
...
# label: idealmodel, spheroid, subspheroid
...
```{r umap_cluster_highlight, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(ggrepel)
library(here)
...
# KMeans 聚类（你可以根据 elbow method 先选 k）
set.seed(123)
...
# Add cluster labels and idealmodel identifier
umap_data <- umap_data %>%
  mutate(
    cluster = factor(kmeans_result$cluster),
    is_idealmodel = if_else(category == "idealmodel", "yes", "no")
  )
...
# Create the plot
ggplot(umap_data, aes(x = x, y = y)) +
  # Regular points (more transparent)
  geom_point(
  ...
  # Highlight idealmodel points (darker, larger)
  geom_point(
...
knitr::kable(data_raw2, caption = "Summary of core typology and morphological parameters.")
