...
mean_spharm_power_summary <- mean_sphericity_tbl |>
  group_by(Group) |>
  summarise(
    mean_spharm_power = mean(`SHE`, na.rm = TRUE),
    sd_spharm_power = sd(`SHE`, na.rm = TRUE)
  ) |>
  ungroup()
...
kruskal_result2 <- kruskal.test(`SHE` ~ Typology, data = data_clean)
...
# Spearman coefficients and p value
...
Overall, the observed relationships between reduction intensity and shape descriptors, particularly mean edge angle and SHE, support the interpretation that spheroid shaping was an efficient and purposeful process. The goal was to produce well-structured, geometrically balanced cores rather than smooth, featureless objects.
