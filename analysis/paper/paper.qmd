...
df_long <- df %>%
  ...
  ungroup() %>%
  mutate(
    category = factor(category, levels = c("spheroids", "cores", "total")),
    material = factor(material, levels = c("Lava", "Dolomite", "Chert", "Other"))
  )

ggplot(df_long, aes(x = material, y = prop, fill = category)) +
  geom_col(position = position_dodge(width = 0.6), width = 0.5) +
  labs(
    title = "Raw Material Proportions in Spheroids, Cores, and Total",
    x = "Raw Material",
    y = "Percentage (%)",
...
ggplot(long_data, aes(x = metric, y = value, fill = artifact)) +
  ...
  scale_fill_manual(values = c(
    "Core" = "#66c2a5", 
    "Spheroid" = "orange", 
    "Polys and Multi cores" = "lightgray"
  )) +
