...
df_long <- df %>%
  ...
  ungroup() %>%
  mutate(
    category = factor(category, levels = c("spheroids","Polys_and_Multi_cores", "cores", "total")),
...
lava_mluti_text <- df_long %>% filter(material == "Lava", category == "Polys_and_Multi_cores") %>% pull(text)
dolomite_mluti_text <- df_long %>% filter(material == "Dolomite", category == "Polys_and_Multi_cores") %>% pull(text)
chert_mluti_text <- df_long %>% filter(material == "Chert", category == "Polys_and_Multi_cores") %>% pull(text)
other_mluti_text <- df_long %>% filter(material == "Other", category == "Polys_and_Multi_cores") %>% pull(text)
```
...
# 绘图，带误差带
ggplot(summary_df_filtered, aes(x = degree, y = mean_power, color = group, fill = group)) +
  ...
  scale_color_manual(values = c(
    "All specimens" = "#66c2a5",
    "Spheroid/Subspheroid" = "orange",
    "Polyhedron/Multifacial" = "lightgray"
  )) +
  scale_fill_manual(values = c(
    "All specimens" = "#66c2a5",
    "Spheroid/Subspheroid" = "orange",
