...
ggplot(umap_data, aes(x = x, y = y)) +
  stat_ellipse(aes(color = cluster), type = "norm", level = 0.95, size = 1) +
  geom_point(aes(color = cluster, size = point_size), alpha = 0.8) +
  geom_text_repel(data = label_data, aes(label = category_lower), size = 4) +
  xlim(x_limits_expanded) +
...
elongation_min <- min(spheroids$E1, spheroids$E2, spheroids$E3, na.rm = TRUE)
elongation_max <- max(spheroids$E1, spheroids$E2, spheroids$E3, na.rm = TRUE)
isotropy_min <- min(spheroids$IS, spheroids$EL, na.rm = TRUE)
isotropy_max <- max(spheroids$IS, spheroids$EL, na.rm = TRUE)
