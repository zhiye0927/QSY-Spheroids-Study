...
ggplot(data_clean, aes(x = Sphericity, y = `SPHARM power`)) +
  geom_point(size = 3, alpha = 0.9, color = "#F28E2B") +
  geom_smooth(method = "lm", se = TRUE, color = "#1C3041", fill = "#A6CEE3") +
...
# 自定义Group名称映射，方便图例显示
df_long <- df_long %>%
  mutate(Group = factor(Group, levels = c("Spheroid + Subspheroid", "Polyhedron + Multifacial"),
                        labels = c("Spheroid + Subspheroid", "Polyhedron + Multifacial")))
...
ggplot(df_long, aes(x = Group, y = Value, fill = Group)) +
  ...
  scale_fill_manual(values = c("Spheroid + Subspheroid" = "orange", "Polyhedron + Multifacial" = "#66c2a5")) +
  labs(x = "", y = "Value", fill = "Group", title = "Sphericity and SHE by Group") +
...
ggplot(df, aes(x = degree, y = variance)) +
  geom_line(color = "#1f78b4", size = 1.3, alpha = 0.8) +          
  geom_point(color = "#ff7f00", size = 3, alpha = 0.9, shape = 21, fill = "#ffbb78", stroke = 1) +  # 橙色边框，填充浅橙
...
# 读取 Excel 文件
file_path <- "E:/Lithic Analysis/QSYSpheroidsStudy/analysis/data/derived_data/umap_l2_l3.XLSX"
