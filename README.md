...
[![Reproducibility
Check](https://github.com/zhiye0927/QSY-Spheroids-Study/actions/workflows/reproducibility.yml/badge.svg)](https://github.com/zhiye0927/QSY-Spheroids-Study/actions/workflows/reproducibility.yml)
...
2.  **Download the compendium**, e.g. from GitHub by clicking on the
    green `< > Code` button above, then click `Download ZIP`, then unzip
    on your computer

3.  **Download STL files** Go to the [OSF
    repository](https://osf.io/ctne9/files/osfstorage) and download the
    folder containing STL models: `3d_models_QSY_spheroids_multi_poly`

4.  **Unzip and place STL files in project directory** Extract the
    downloaded archive. After unzipping, copy the entire folder
    **`3d_models_QSY_spheroids_multi_poly`** into this directory:
    `/py_SPHARM/SPHARM_main`

5.  Set your terminal working directory to the compendium, and run this
    line to **build the container** on your computer, it will take a few
    minutes and require a fast internet connection. (be sure to include
    the period):
...
6.  **Start the Docker container** on your computer by running this line
    in your terminal (linux and OSX):
...
7.  Go to <http://localhost:8787/> with your browser and enter `rstudio`
    in both fields to **start RStudio in your browser** All of the
    following steps must be done in this instance of RStudio in the
    browser, using either the bash terminal or the R console.

8.  **Create a new Python environment** in RStudio in your browser by
    typing this in the RStudio terminal. This will move to the directory
    in the repository that has the Python code, open a terminal window,
    and create a new Python environment for this project to avoid
    conflicts with Python packages you may already have, then we will
...
9.  **Run the batch operation script** This script will process all STL
    files in the folder `3d_models_QSY_spheroids_multi_poly` and perform
...
10. **Run the curvature script** This script will compute the curvature
    for all STL files in the folder
...
11. **Run the sphericity script** This script will compute the
    sphericity for all STL files in the folder
    `3d_models_QSY_spheroids_multi_poly`. By default, all output results
    will be saved to
    `/analysis/data/raw_data/SPHARM_sphericity_curvature_result`
...
12. **Data processing and visualization in R** Our manuscript is a
    Quarto document that contains the text of our paper interwoven with
    blocks of R code that generates the data visualizations and tables.
    To execute all the code and generate a docx document showing the
    results, run this line in the terminal, assuming you are continuing
...
Alternatively, you can open `analysis/paper/paper.qmd` in RStudio in
your browser and render it with
`quarto::quarto_render("analysis/paper/paper.qmd")` to produce
`paper.docx`, or run the R code interactively.

13. After the code has executed, view the rendered output document by
    running `browseURL("analysis/paper/paper.docx")`

14. When finished, to delete all the Docker containers from your
