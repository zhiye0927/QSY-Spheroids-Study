...
def plot_sphericity_by_core_shape_with_stats(excel_path):
    """
    从Excel文件加载核心类型与球度数据，绘制箱线图并执行 Kruskal-Wallis 检验。

    参数：
    excel_path : str
        Excel文件路径，需包含 "Core shape" 和 "Sphericity" 两列。
    """
    # 读取并清洗数据
    df = pd.read_excel(excel_path)
    df.columns = df.columns.str.strip().str.lower()
    df.rename(columns={'core shape': 'label', 'sphericity': 'sphericity'}, inplace=True)

    # 执行 Kruskal-Wallis H 检验
    groups = [group['sphericity'].dropna().values for name, group in df.groupby('label')]
    stat, p_value = kruskal(*groups)

    # 打印结果
    print(f"Kruskal-Wallis H 检验结果：H = {stat:.4f}, p = {p_value:.4e}")

    # 绘制箱线图
    plt.figure(figsize=(10, 6))
    sns.boxplot(x='label', y='sphericity', data=df)
    plt.xticks(rotation=45)
    plt.title("球度在不同石核类型间的分布")
    plt.ylabel("Sphericity")
    plt.xlabel("Core Shape")

    # 添加 p 值注释
    plt.figtext(0.15, 0.92, f"Kruskal-Wallis H = {stat:.2f}, p = {p_value:.2e}",
                fontsize=10, ha='left', color='darkred')

    plt.tight_layout()
    plt.show()

#plot_sphericity_by_core_shape_with_stats(r"E:\spheroids_analysis\core_shape and sphericity.xlsx")
