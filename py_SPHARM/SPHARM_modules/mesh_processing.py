...
def visualize_with_lighting_and_wire(normalized_vertices, decimated_faces):
    ...
    plotter.set_background('white')  # 白背景适合发表

    # 添加带光照的浅色面（phong shading 默认开启）
    plotter.add_mesh(
        mesh,
        color='#f2f2f2',        # 超浅灰色
        opacity=0.5,            # 半透明
    ...
    plotter.add_mesh(
        ...
        color='#999999',        # 浅灰色线框
        line_width=1.2,
        lighting=False          # 线框不需要光照
    )

    plotter.add_axes()
    plotter.add_title("归一化网格 + 光照 + 线框", font_size=12)
    plotter.show()